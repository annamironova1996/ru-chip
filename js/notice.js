  ...
  window.JNoticeSurface.prototype = {
    ...
    getItemImg: function (options) {
      ...
      if (image) {
        ...
        if (!BX.hasClass(image, "lazyloaded")) {
        ...
        if (!src) {
          let dataBg = this.bUseJquery ? jQuery(image).data("bg") : BX.data(image, "bg");
          if (dataBg) {
            src = dataBg;
          }
        }
      ...
      } else if ((image = item.querySelector(".image-list-wrapper img, .js-notice-block__image img"))) {
        src = image.getAttribute("src");
        if (!BX.hasClass(image, "lazyloaded")) {
          let dataSrc = this.bUseJquery ? jQuery(image).data("src") : BX.data(image, "src");
          if (dataSrc) {
    ...
    getMoreContent: function (items) {
      ...
      if (items.length > 1) {
        ...
        for (let cnt = items.length, i = 1; i < cnt; ++i) {
          if (
            items[i] &&
            typeof items[i] === "object" &&
            items[i].itemAction &&
            typeof JItemActionService === "function" &&
            items[i].itemAction instanceof JItemActionService
          ) {
        ...
        if (products.length) {
          ...
          let titles = [BX.message("NOTICE_PRODUCT1"), BX.message("NOTICE_PRODUCT2"), BX.message("NOTICE_PRODUCT0")];
          let cases = [2, 0, 1, 1, 1, 2];
          moreText += BX.message("NOTICE_MORE")
            .replace("#COUNT#", cnt)
            .replace("#PRODUCTS#", titles[cnt % 100 > 4 && cnt % 100 < 20 ? 2 : cases[Math.min(cnt % 10, 5)]]);
        }

        if (services.length) {
          let cnt = services.length;
          let titles = [BX.message("NOTICE_SERVICE1"), BX.message("NOTICE_SERVICE2"), BX.message("NOTICE_SERVICE0")];
          let cases = [2, 0, 1, 1, 1, 2];
          moreText +=
            (moreText ? "<br />" : "") +
        ...
        if (moreText) {
          moreText = '<div class="notice__detail__more fw-600">' + moreText + "</div>";
        }
      }
    ...
    onAdd2Cart: function (items, state) {
        ...
        if (typeof items === "object" && items.length) {
          for (i in items) {
            ...
            if (item instanceof Node) {
              if (item.classList.contains("js-external-button")) {
                item = document.querySelector(".catalog-detail");
              } else {
                item = item.closest(".js-popup-block");
              }
    ...
    onAdd2Basket: function (items, state) {
      return this.onAdd2Cart(...arguments);
    },

    onAdd2Service: function (items, state) {
      return this.onAdd2Cart(...arguments);
    },
  ...
  window.JNotice.prototype = {
      ...
      if (typeof content === "object") {
        ...
        if (title.length || detail.length || image.length) {
          ...
          if (title || detail) {
            ...
            if (title) {
              content += '<div class="notice__title switcher-title fw-600">' + title + "</div>";
            }
            if (detail) {
    ...
    init: function () {
      if (!this.inited) {
        ...
        var closer = "";
        if (this.closeable) {
          closer = `<span class="notice__closer stroke-theme-hover" title="${BX.message("NOTICE_CLOSE")}">
